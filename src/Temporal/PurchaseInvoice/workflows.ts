...
export async function syncFailedAccurate(): Promise<Object> {
  try {
    const failedSync = await callFailedAccurate();
    let resultFailedSync = Object(failedSync)
    if (resultFailedSync.status != 'success') {
      return {
        'status': 'error',
    ...
    for (let index = 0; index < resultFailedSync.data.length; index++) {
        const products: Item[] = resultFailedSync.data[index].items
          const data: PurchaseInvoice = {
              purchase_invoice: resultFailedSync.data[index].id_purchase_invoice,
              purchase_order: resultFailedSync.data[index].id_po_accurate,
              supplier: resultFailedSync.data[index].id_supplier_accurate,
              transaction_date:  format(new Date(resultFailedSync.data[index].posting_date), 'dd/MM/yyyy'),
              items: products
        }
        const deleteData = await deleteFailedAccurate(data.id || 0)
